...
class DENTModel(nn.Module):
    def __init__(self, model):
        ...
        from model.dent import Dent
        self.dent = Dent(model)
        self.model = model
...
class Dent(nn.Module):
    ...
    def __init__(self, model):
        super().__init__()
        # self.model = convert_batchnorm(model, SampleAwareStaticBatchNorm2d) # opt_cfg.BN_FUN
        self.model = convert_batchnorm(model, SampleAwareOnlineBatchNorm2d)
        #self.opt_cfg = opt_cfg
        self.steps = 6#opt_cfg.STEPS
        self.criterion = tent#globals()[opt_cfg.LOSS]
        assert self.steps > 0, "dent requires >= 1 step(s) to forward and update"

    def forward(self, x, return_model=False):
