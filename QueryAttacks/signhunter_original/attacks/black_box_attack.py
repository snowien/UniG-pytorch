...
from QueryAttacks.signhunter_original.utils.compute_fcts import l2_proj_maker, linf_proj_maker
...
class BlackBoxAttack(object):
    ...
    def run(self, xs, loss_fct, early_stop_crit_fct):
        """
        ...
        while True:
            ...
            its += 1
            if its % 10 == 0:
                print(f"its:{its} dones:{np.mean(dones_mask)}")
                sys.stdout.flush()
            self.is_new_batch = False
        ...
        print(f"queries: {num_loss_queries * correct_classified_mask}")
        sys.stdout.flush()
        # set self._proj to None to ensure it is intended use
        self._proj = None

        return logs_dict, xs_t
