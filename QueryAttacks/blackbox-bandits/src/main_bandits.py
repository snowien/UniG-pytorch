...
def make_adversarial_examples(image, true_label, args, model_to_fool, IMAGENET_SL):
    ...
    proj_step = proj_maker(image, args.test_eps)
    ...
    t = 0
    print('max query: ', args.max_query)
    while not ch.any(total_queries > args.max_query):
        ...
        if args.mode == 'l2':
            if not ch.all(norm(image - orig_images) <= args.test_eps + 1e-3):
                pdb.set_trace()
        else:
            if not (image - orig_images).max() <= args.test_eps + 1e-3:
                pdb.set_trace()
        ...
        # print(correct_classified_mask.sum())
...
def main(args, model_to_fool, dataset_loader):
    ...
    for i, (images, targets) in enumerate(dataset_loader):
        if i*args.batch_size >= args.total_images:
            break
        res = make_adversarial_examples(images.cuda(), targets.cuda(), args, model_to_fool, 224 if args.dataset == 'imagenet' else 32)
...
def bandit_attack(args, model_to_fool, test_loader):
    args_dict = None
    ...
    with ch.no_grad():
        main(args, model_to_fool, test_loader)
    return
