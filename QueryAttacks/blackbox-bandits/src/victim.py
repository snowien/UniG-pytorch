...
class AAASimple(nn.Module):
    def __init__(self, dataset, arch, norm, model_dir, 
        ...
        self.arch_ori = arch
        self.arch = '%s_AAA-Lr-%.2f-Ai-%d' % (self.arch_ori, self.reverse_step, self.attractor_interval)
    ...
    def forward(self, x):
        ...
        for counter in range(n_batches):
            ...
            prob_max_ori = F.softmax(logits_ori, dim=1).max(1)[0]
            value, index = torch.topk(logits_ori, k=2, dim=1)
            ...
            diff = (margin_ori - target).abs()

            with torch.enable_grad():
                logits.requires_grad = True
                optimizer = torch.optim.Adam([logits], lr=self.optimizer_lr)
                #while diff.mean() > self.reverse_step * self.attractor_interval / 2 * 0.1:
                for i in range(self.num_iter):
                    ...
                    diff = (margin - target).abs()
                    loss_defense = diff.mean()
                    
                    loss = loss_calibration * self.calibration_loss_weight + loss_defense
                    optimizer.zero_grad()
                    loss.backward()
                    optimizer.step()
...
class AAARescale(nn.Module):
    def __init__(self, dataset, arch, norm, model_dir, 
        ...
        super(AAARescale, self).__init__()
        self.dataset = dataset
        try: 
            self.cnn = getattr(torchvision.models, arch)(pretrained=True).to(device).eval()
            self.mean = [0.485, 0.456, 0.406]
        ...
        self.num_iter = num_iter
        self.arch_ori = arch
        self.arch = '%s_AAAR-Lr-%.1f-Ai-%d-Cw-%d' % (self.arch_ori, self.reverse_step, self.attractor_interval, self.calibration_loss_weight)
